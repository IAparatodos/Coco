...
El protagonista es "Coco", una perrita Bichón Maltés blanca, pequeña, peluda y muy curiosa.
...
const storySchema: Schema = {
  ...
  properties: {
    ...
    imagePrompt: {
      type: Type.STRING,
      description: "Una descripción visual detallada de la escena para generar una imagen. Describe a la perrita Coco y el entorno de la tienda de azulejos.",
    },
    choices: {
      type: Type.ARRAY,
      items: {
...
export const generateStorySegment = async (history: string, userChoice: string): Promise<Omit<StorySegment, 'id' | 'imageData'>> => {
  try {
    const prompt = `
      ...
      Asegúrate de que la trama avance. Coco debe interactuar con elementos de la tienda de azulejos (baldosas, grifos, dependientes, cajas).
    `;
    ...
    return {
      narrative: "Coco se ha despistado un poco oliendo una baldosa hidráulica muy bonita. (Error de conexión)",
