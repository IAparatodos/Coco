/**
 * JavaScript para Categoría Porcelánico Extrafino (ID: 2510)
...
(function($) {
     ...
     */
    function moverBotonFiltroExtrafino() {
        var $boton = $('.wpc-filters-open-button-container');
        var $destino = $('#destino-filtro-adria-extrafino');
        ...
        if ($boton.length && $destino.length) {
            ...
            $elemento.find('a').text('FILTRAR EXTRAFINO');
    ...
    function scrollSuaveAlCatalogo() {
        var $catalogo = $('#catalogo-extrafino-foco');
    ...
    $(document).ready(moverBotonFiltroExtrafino);
    
    // Backup: ejecutar después de 1.5s por si el DOM no estaba listo
    setTimeout(moverBotonFiltroExtrafino, 1500);
