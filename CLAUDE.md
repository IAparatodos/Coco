...
### 7. NUNCA ocultar bloques WooCommerce de forma GLOBAL para todas las categor&iacute;as
- **Error**: En `functions-server.php` se a&ntilde;adi&oacute; `add_action('wp_head', 'adrihosan_ocultar_filtros_legacy', 5)` FUERA del `switch` pero DENTRO del `if (is_product_category())`. Esto ocultaba `.woocommerce-products-header`, `.term-description`, `.wd-shop-tools`, etc. en **TODAS** las categor&iacute;as de producto, no solo en las que tienen landing personalizada. Las categor&iacute;as sin landing se quedaron sin t&iacute;tulo, descripci&oacute;n ni herramientas de filtrado - solo se ve&iacute;a el loop de productos.
- **Causa**: La funci&oacute;n `adrihosan_ocultar_filtros_legacy()` se registr&oacute; dentro del bloque `is_product_category()` del master controller, pero antes o despu&eacute;s del `switch`, aplicando a todas las categor&iacute;as.
- **Regla**: Los estilos CSS que ocultan elementos est&aacute;ndar de WooCommerce SOLO deben a&ntilde;adirse **dentro de cada funci&oacute;n `setup_*_cpu_fix()` espec&iacute;fica**. NUNCA fuera del `switch`, ni como funci&oacute;n global, ni como acci&oacute;n registrada para todas las categor&iacute;as.
- **Correcto**:
  ```php
  function adrihosan_setup_ejemplo_cpu_fix() {
      // ... hooks espec&iacute;ficos ...
      add_action('wp_head', function() {
          echo '<style>.wd-shop-tools, .advanced-filter { display: none !important; }</style>';
      });
  }
  ```
- **INCORRECTO** (provoca bug en todas las categor&iacute;as):
  ```php
  function adrihosan_master_controller_cpu_fix() {
      if (!is_product_category()) return;
      // !! ESTO AFECTA A TODAS LAS CATEGORÍAS !!
      add_action('wp_head', 'adrihosan_ocultar_filtros_legacy', 5);
      $cat_id = get_queried_object_id();
      switch($cat_id) { ... }
  }
  ```
- **Motivo**: Las categor&iacute;as que NO tienen `case` en el switch necesitan los bloques est&aacute;ndar de WooCommerce (t&iacute;tulo, descripci&oacute;n, filtros, breadcrumb). Si se ocultan globalmente, esas categor&iacute;as quedan rotas.
...
### 2026-03-13
- **CLAUDE.md**: Añadido error #7 - NUNCA ocultar bloques WooCommerce de forma global
  - Documenta el bug donde `adrihosan_ocultar_filtros_legacy()` se registraba fuera del switch afectando a todas las categorías
  - Incluye ejemplos correcto vs incorrecto
- **Categoría 2358 (Azulejos Antiguos)**: Actualizados enlaces de pills y hero image
  - Cocinas, Baños, Rústico, Hidráulico, Vintage → URLs reales de subcategorías
  - Retro → renombrado a "Biselado" con URL correspondiente
  - Desgastado → renombrado a "Básicos" con URL correspondiente
  - Hero image actualizada
