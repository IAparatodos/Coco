...
if (!$class) {
	...
	if ($block_id) {
		$footer_block_html = get_transient('dw_footer_block_html');
		if ($footer_block_html === false) {
			$get_block = get_post( $block_id );
			$footer_block_html = apply_filters('the_content', $get_block->post_content);
			set_transient('dw_footer_block_html', $footer_block_html, HOUR_IN_SECONDS);
		}
		echo '<div class="custom-block">' . $footer_block_html . '</div>';
	}
	if (is_front_page()) {
		$block_id_home = get_option('dw-op-block-footer-home');
		if ($block_id_home) {
			$footer_home_html = get_transient('dw_footer_block_home_html');
			if ($footer_home_html === false) {
				$get_block = get_post( $block_id_home );
				$footer_home_html = apply_filters('the_content', $get_block->post_content);
				set_transient('dw_footer_block_home_html', $footer_home_html, HOUR_IN_SECONDS);
			}
			echo '<div class="custom-block-home">' . $footer_home_html . '</div>';
		}
	}
}
?>
